...
cal.tiny <- calibrate(tiny$cra,tiny$cra.error)

# Bootstrapping ----
boot.large <- sampleDates(cal.large,nsim=1000,boot=TRUE)
boot.small <- sampleDates(cal.small,nsim=1000,boot=TRUE)
boot.tiny <- sampleDates(cal.tiny,nsim=1000,boot=TRUE)

# Composite KDE ---
ckde.large <- ckde(boot.large,timeRange=c(7000,3000),bw=50)
ckde.small <- ckde(boot.small,timeRange=c(7000,3000),bw=50)
ckde.tiny <- ckde(boot.tiny,timeRange=c(7000,3000),bw=50)

save(ckde.large,ckde.small,ckde.tiny,file=here('results','ckde_res.RData'))

...
set_rc_meas(here('results'),"baydem_tiny_res",tiny.data)
analysis <- readRDS(here('results','baydem_tiny_res.rds'))
...
set_density_model(here('results'),"baydem_tiny_res",density_model)
set_calib_curve(here('results'),"baydem_tiny_res","intcal20")
analysis <- readRDS(here('results','baydem_tiny_res.rds'))
hp <- list()
...
do_bayesian_inference(here('results'),"baydem_tiny_res",hp,control=list(samps_per_chain=5000))

# Bayesian Summary
do_bayesian_summary(here('results'),"baydem_tiny_res")
...
baydem.tiny <- readRDS(here('results','baydem_tiny_res.rds'))
bestK.tiny <- baydem.tiny$K_best
baydem.tiny.plot <- data.frame(CalBP=BCADtoBP(baydem.tiny$bayesian_summary$tau))
ind.lo.tiny <- which(baydem.tiny$bayesian_summary$probs == 0.025)
ind.hi.tiny <- which(baydem.tiny$bayesian_summary$probs == 0.975)
ind.m.tiny <- which(baydem.tiny$bayesian_summary$probs == 0.5)
baydem.tiny.plot$lo <- baydem.tiny$bayesian_summary$Qdens[ind.lo.tiny,]
baydem.tiny.plot$hi <- baydem.tiny$bayesian_summary$Qdens[ind.hi.tiny,]
baydem.tiny.plot$m <- baydem.tiny$bayesian_summary$Qdens[ind.m.tiny,]
...
save(bestK.tiny,bestK.small,bestK.large,baydem.tiny.plot,baydem.small.plot,baydem.large.plot,file=here('results','baydem_res.RData'))
